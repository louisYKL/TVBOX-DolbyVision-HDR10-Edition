...
- 最终热修复解决首帧后停住：缓存边界的临时空 `206` 或提前结束响应会自动重开 Range，不再误报 EOF；32 位端以 4 MB 分块维持 24 MB 启播目标和 32 MB 前向缓存，并停止写入运行日志文件。
- 视频打开后立即预读取，后台持续维持大缓存但不再用缓存阈值阻塞起播；准备、缓冲和拖动阶段显示真实百分比与网速。
- 初始历史进度和普通拖动统一使用单一 seek 状态机，连续快速拖动时新目标立即覆盖旧目标且不排队播放中间位置，修复 seek 后卡住、旧进度回灌、重复 seek、假完成和误跳下一集。
