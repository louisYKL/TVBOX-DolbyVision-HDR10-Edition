# Changelog

## 0.1.9.1

- 将 `java32`、`java64`、`hisense32` 三条线统一同步到 `0.1.9.1` 并重新发布。
- 修复点播视频播放过程中偶发自动卡住时，底部菜单栏进度错误跳到片尾、与实际画面不一致的问题。
- 重构点播进度恢复链路，分离真实播放位置和待恢复 seek 目标，避免异常进度污染历史记录。
- 增加脏恢复进度清理逻辑，避免旧缓存把视频误判为已看完或一开播就进度错乱。
- release 资产继续提供三端 APK、详细更新日志和 SHA256 校验清单。

## 0.1.8

- 将 `java32`、`java64`、`hisense32` 三条线统一同步到 `0.1.8` 并重新发布。
- 继续修复点播切源、快进卡顿、偶发卡死和进度误保存问题。
- `java64` 保持已修复的直播黑屏问题不回退，并继续维持三条线独立边界。
- release 资产继续提供三端 APK、详细更新日志和 SHA256 校验清单。

## 0.1.7

- 将 `java32`、`java64`、`hisense32` 三条线统一同步到 `0.1.7` 并重新发布。
- 修复点播切源、快进过程中的卡顿和卡死问题。
- 修复 `java64` 直播黑屏问题，并保持三条线的代码边界独立。
- release 资产继续提供三端 APK、详细更新日志和 SHA256 校验清单。

## 0.1.5

- 将 `java32`、`java64`、`hisense32` 三条线统一同步到 `0.1.5` 并重新发布。
- 三端 APK 继续独立构建、独立打包、独立上传。
- 本次 release 只做版本同步和发布资料补齐，不改播放功能。

## 0.1.4

- 将 `TVBoxOS-main`、`TVBoxOS-java64`、`TVBoxOS-hisense` 三个目录作为一个 GitHub 源码仓统一发布。
- 重新构建并发布三端 `0.1.4` 正式 APK。
- 保留三条线各自独立的源码边界，不把现有代码硬并成一个混合 flavor 工程。
- release 资产补齐 APK、详细更新日志和 SHA256 校验文件。
- `java64` 当前直播黑屏问题保留为已知问题，没有在 `0.1.4` 里伪装成已修复。

## 0.1.3

- 将 `java32`、`java64`、`hisense32` 三个 Android 包统一整理到同一个 release tag 下发布。
- 收紧电视向分支的 Dolby Vision / HDR 路由逻辑。
- 改进 `java64` 的全屏渲染和触控交互链路。

## 0.1.1

- 收紧本地代理 MKV / HDR / Dolby Vision 的预探测和路由判断。
- 修复切源、重播和进度恢复相关的播放状态串扰。

## 0.1

- 建立三条发布线的最初版本模型：`java32`、`java64`、`hisense32`。
