...
- Java64 继续保持 `arm64-v8a`、触控/手势/焦点链、字幕/音频/HDR 路由和原签名证书。三端统一为 `0.2.7`、`versionCode 2070`，可覆盖安装 `0.2.6`。
...
- Java64 retains its `arm64-v8a` ABI, touch/gesture/focus chain, subtitle/audio/HDR routing, and existing signing certificate. All variants are `0.2.7` with `versionCode 2070`, installable over `0.2.6`.
