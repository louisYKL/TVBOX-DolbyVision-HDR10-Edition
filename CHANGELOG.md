...
- 三端继续保留设备硬解、立体声 PCM、HDR 字幕、Java64 触控/手势/焦点链、海信独立包名/ABI 和原签名证书。统一为 `0.2.8`、`versionCode 2080`，可覆盖安装 `0.2.7` 及更早版本。
...
- All variants retain device hardware decode, stereo PCM, HDR subtitles, the Java64 touch/gesture/focus chain, the independent Hisense package/ABI, and the existing signing certificate. Version `0.2.8` (`versionCode 2080`) installs over `0.2.7` and earlier releases.
