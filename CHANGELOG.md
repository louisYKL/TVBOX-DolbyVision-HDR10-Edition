...
- 修复直播"有声音无画面"：部分设备在直播/HLS 起播、首个关键帧到达前会瞬时上报系统信息码 `MEDIA_INFO_VIDEO_NOT_PLAYING`（805，厂商状态如 `-38`），此前被当作首帧失败直接置 `STATE_ERROR` 并强制换源，而音频已在播放，于是出现有声音无画面并不断换源；现在 805 不再直接报错，改为交给已就绪、感知缓冲的首帧看门狗判定——瞬时 805 不会再杀掉即将出画的直播源，真正无视频的源仍会在看门狗超时后被换源。
- 三端（java32、java64、Hisense）共用同一套修复。版本统一为 `0.2.3`（`versionCode 2028`），沿用原签名证书，可覆盖安装 `0.2.2` 及更早的 `0.2.1.x`。
...
- Fixed live "audio but no video": some devices transiently report `MEDIA_INFO_VIDEO_NOT_PLAYING` (805, with vendor status such as `-38`) during live/HLS startup before the first keyframe. This was treated as a first-frame failure that forced `STATE_ERROR` and a channel reswitch while audio was already playing, producing audio-only playback and a reswitch loop. 805 no longer hard-fails; it defers to the already-armed, buffering-aware first-frame watchdog, so a spurious 805 can't kill a stream that is about to render, while a truly video-less stream is still caught by the watchdog deadline.
- Fixed live "audio but no video": many devices transiently raise `MEDIA_INFO_VIDEO_NOT_PLAYING` (805) with vendor status codes such as `-38` during live/HLS startup, before the first keyframe arrives. Treating any 805 as a fatal first-frame failure forced perfectly good live channels into STATE_ERROR and an endless channel-reswitch loop while audio was already playing — exactly "audio but no video". 805 is now only a soft signal handed to the buffering-aware render watchdog; a genuinely video-less stream is still caught after the watchdog's real timeout.
- Shipped across all three variants (java32, java64, Hisense). Unified under `0.2.3` (`versionCode 2028`) with the existing signing certificate for in-place updates from `0.2.2` and earlier `0.2.1.x`.
