...
- 海信继续保留独立包名 `com.github.tvbox.osc.hisense`、`armeabi-v7a` ABI、设备硬解、立体声 PCM、HDR 字幕和原签名证书。三端统一为 `0.2.8`、`versionCode 2080`，可覆盖安装 `0.2.7` 及更早版本。
...
- Hisense retains the independent `com.github.tvbox.osc.hisense` package, `armeabi-v7a` ABI, device hardware decode, stereo PCM, HDR subtitles, and existing signing certificate. All variants use `0.2.8` (`versionCode 2080`) and install over `0.2.7` and earlier releases.
