...
- Java64 retains its `arm64-v8a` ABI, touch/gesture/focus chain, device hardware decode, stereo PCM audio, and HDR subtitles. The existing signature supports in-place installs over `0.2.7` and earlier releases.
...
- The Java64 build keeps its `arm64-v8a` ABI, touch/gesture/focus chain, subtitle/audio/HDR routing, and existing signing certificate. It can be installed over `0.2.6`.
